<template>
  <div class="app-container">
    <el-row :gutter="20">
      <el-col>
        <el-button type="info" class="m" @click="Add">新增專案類別</el-button>
      </el-col>
      <el-col>
        <el-form :inline="true" class="demo-form-inline">
          <el-form-item label="搜尋">
            <el-input v-model="searchMap.word" placeholder="類別 ENG..." />
          </el-form-item>
          <el-form-item>
    ...
    <!-- Form -->
    <el-dialog title="專案類別管理" :visible.sync="dialogFormVisible">
      <el-form ref="form" :model="form" :top="true" :rules="rules">
        <el-form-item label="類別" prop="name">
          <el-input
            v-model="form.name"
            autocomplete="off"
            placeholder="類別"
        ...
        <el-form-item label="類別(ENG)" prop="eng_name">
          <el-input
  ...
  data() {
    return {
      ...
      rules: {
        ...
        eng_name: [
          { required: true, message: '請輸入類別(ENG)!', trigger: 'blur' },
          { max: 64, message: '長度不能超過64個字!', trigger: 'blur' }
        ],
        img: [
  ...
  methods: {
    ...
    Search() {
      ...
      if (this.searchMap.name !== null) {
        this.list = this.fullList.filter(array => array.eng_name.match(this.searchMap.word) || array.name.match(this.searchMap.word))
      } else {
    ...
    handleRemove(file, fileList) {
      ...
      removeProjectsTypeFile(form).then(res => {
        if (res.code === 200) {
          if (this.mode === 'edit') {
    ...
    Add() {
      if (this.mode === 'edit') {
        this.mode = 'add'
        this.form = { id: '', name: '', eng_name: '', img: '', orders: 0, status: 'published' }
        this.fileList = []
      }
      this.Switch()
    },
    ...
    Insert() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.Switch()
          insertProjectsType(this.form).then(res => {
            if (res.code === 200) {
    ...
    Update() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.Switch()
          updateProjectsType(this.form).then(res => {
            if (res.code === 200) {
              this.resSuccess(res.message)
    ...
    Delete(id) {
      this.$confirm(`確定要刪除嗎？`)
        .then(() => {
          ...
          deleteProjectsType(this.form).then(res => {
            if (res.code === 200) {
              this.resSuccess(res.message)
              this.form = { id: '', position: '', name: '', avatar: '', status: 'published' }
              this.fileList = []
